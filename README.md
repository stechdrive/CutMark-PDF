...
- 配置済み番号のドラッグ移動
- 選択中番号の削除
- 番号を 1 つ選択して `選択カット以降を再採番` を押すと、その番号以降を現在の採番設定で振り直せる
- `Ctrl+Z / Cmd+Z` で Undo
...
- 修正版素材でページ差し替えや増減があった場合は、割付調整後に対象カットを選んで `選択カット以降を再採番` すると番号を揃え直せる
...
- 番号順を直したい場所のカットを選び、`選択カット以降を再採番` で後続番号を揃え直す
- `適用` 後に書き出し
