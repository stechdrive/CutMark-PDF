...
const getCutLabelSummary = (
  cuts: Array<{ label: string }>,
  options: { maxLabels?: number } = {}
) => {
  const labels = cuts
    .map((cut) => cut.label.trim())
  ...
  return `${visibleLabels.join('・')}${labels.length > maxLabels ? '…' : ''}`;
};
...
export const ProjectOrganizerPanel: React.FC<ProjectOrganizerPanelProps> = ({
  ...
  const unplacedLogicalPageCount = organizer.unplacedLogicalPageCount;

  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-sky-200 bg-sky-50/70 p-3">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="text-[11px] font-semibold tracking-wide text-sky-700">読込中プロジェクト</div>
          <div className="text-[13px] font-bold leading-4 text-slate-900 break-all">{projectName}</div>
          <div className="text-[10px] leading-4 text-slate-500">
            保存: {new Date(savedAt).toLocaleString('ja-JP')}
          </div>
        </div>
        <span className="shrink-0 rounded-full bg-white px-2 py-0.5 text-[10px] font-semibold text-slate-700 shadow-sm">
          {statusLabel}
        </span>
      </div>

      <div className="mt-2.5 shrink-0 border-b border-sky-100/80 pb-2.5">
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={onApplyProject}
            disabled={!canApplyProject}
            className="flex-1 rounded-lg bg-sky-600 px-3 py-1.5 text-[13px] font-medium text-white transition-colors hover:bg-sky-500 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500"
          >
            適用
          </button>
          {canResetBindings && (
            <button
              ...
              className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-sky-200 bg-white px-2 py-1.5 text-[10px] font-medium text-sky-700 transition-colors hover:border-sky-300 hover:bg-sky-50 hover:text-sky-800"
            >
            ...
            {organizer.slots.map((slot) => {
              ...
              return (
                <div
                  ...
                  className={`relative overflow-hidden rounded-xl border p-2 transition-colors ${
                ...
                >
                  {isActiveContePage && (
                    <div className="pointer-events-none absolute inset-y-2 left-0 w-1 rounded-r-full bg-sky-500" />
                  )}
                  ...
                  <div className="mb-1.5 flex items-center justify-between gap-2 pl-1">
                    ...
                    <div className="relative flex shrink-0 items-center gap-1">
                      <button
                        ...
                        aria-label={`Page ${slot.contePageNumber} に空欄を挿入`}
                        className="rounded border border-sky-100 p-1 text-slate-600 hover:bg-sky-50"
                        title="ここに空欄ページを挿入"
                      >
                      ...
                      <button
                        ...
                        aria-label={`カット番号ページの削除方法を選ぶ Page ${slot.contePageNumber}`}
                        className="inline-flex items-center gap-1 rounded border border-sky-100 px-1.5 py-1 text-rose-600 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-40"
                  ...
                  <div className="flex items-start gap-1.5">
                    <div
                      ...
                      className={`flex min-w-0 flex-1 items-center gap-1.5 rounded-lg border px-2 py-1 transition-colors ${
                    ...
                    >
                      ...
                      >
                        <div
                          className="max-h-[2rem] overflow-hidden pr-1 text-[11px] font-semibold leading-4 tracking-tight whitespace-normal break-all"
                          title={slot.logicalPage ? getCutLabelSummary(slot.logicalPage.cuts) : undefined}
                        >
                          {slot.logicalPageId
                            ? getCutLabelSummary(slot.logicalPage?.cuts ?? [], { maxLabels: 5 })
                            : '未割付'}
                        </div>
                        {!slot.logicalPageId && (
                          <div className="truncate text-[10px] leading-4 text-current/75">
                            ここにドラッグして割り付け
                          </div>
                        )}
                      </button>
                  ...
                  <button
                    ...
                    className="mt-1 block w-full rounded-lg border border-slate-200 bg-slate-50/80 px-2 py-1 text-left text-[10px] leading-4 text-slate-600 hover:border-sky-200 hover:bg-sky-50/60"
                  >
                    <div className="break-all text-slate-700">
                      <span className="font-medium">現在:</span> {formatAssetLabel(slot.asset)}
                    </div>
                    {slot.logicalPage && slot.expectedAsset && (
                      <div className="mt-0.5 break-all text-slate-500">
                        <span className="font-medium">前回:</span> {formatAssetLabel(slot.expectedAsset)}
                      </div>
                    )}
                  </button>
                </div>
              ...
              <div className="space-y-2">
                {organizer.unplacedPages.map((page) => (
                  <div
                    ...
                    className={`flex cursor-grab items-center gap-2 rounded-lg border px-2.5 py-1.5 transition-colors active:cursor-grabbing ${
                      page.isSelected
                        ? 'border-sky-400 bg-sky-100/70'
                  ...
                  >
                    ...
                    >
                      <div
                        className="max-h-[2rem] overflow-hidden pr-1 text-[11px] font-semibold leading-4 tracking-tight whitespace-normal break-all text-slate-800"
                        title={getCutLabelSummary(page.logicalPage.cuts)}
                      >
                        {getCutLabelSummary(page.logicalPage.cuts, { maxLabels: 5 })}
                      </div>
                      <div
                      ...
                      >
                        {getCutLabelSummary(page.logicalPage.cuts, { maxLabels: 5 })}
                        {page.expectedAsset ? ` / 前回: ${formatAssetLabel(page.expectedAsset)}` : ''}
                      </div>
                    </button>
                    <button
                      type="button"
