...
export const DocumentPreviewHud: React.FC<DocumentPreviewHudProps> = ({
  ...
  const mobileZoomBottom = 'calc(var(--mobile-dock-h, 0px) + 4.5rem)';
  const mobilePagerBottom = 'calc(var(--mobile-dock-h, 0px) + 0.75rem)';
    ...
    <>
      <div
        className={`absolute z-40 flex border border-gray-200 bg-white shadow-lg ${
          isMobileLayout
            ? 'left-3 gap-1.5 rounded-xl p-1.5'
            : 'bottom-6 left-6 gap-2 rounded-lg p-2'
        }`}
        style={isMobileLayout ? { bottom: mobileZoomBottom } : undefined}
      >
        <button
      ...
      <div
        className={`fixed left-1/2 z-40 flex -translate-x-1/2 flex-col items-center ${
          isMobileLayout ? 'gap-1.5' : 'bottom-4 gap-2'
        }`}
        style={isMobileLayout ? { bottom: mobilePagerBottom } : undefined}
      >
        <div
